...
export class Slider extends Focusable {
  ...
  private tickLabelElements: TickLabel[] = []
  ...
  private onSlotChange(event: Event): void {
    ...
    this.updateTickLabels(slot)
  }

  private updateTickLabels(slot: HTMLSlotElement): void {
    const nodes = slot.assignedNodes()
    this.tickLabelElements = nodes.filter(
      (node) => (node as HTMLElement) instanceof TickLabel
    ) as TickLabel[]
    this.requestUpdate()
  }
  ...
  private tickLabelText(i: number): string {
    const tickStep = this.tickStep || this.step

    if (this.tickLabelElements.length) {
      return this.tickLabelElements[i]?.textContent || `${i * tickStep}`
    }
    return `${i * tickStep}`
  }

  private tickLabelClassNames(i: number): string {
    if (this.tickLabelElements[i]?.hasAttribute('highlight')) {
      return 'tickLabel highlight'
    } else {
      return 'tickLabel'
    }
  }
        ...
        ${ticks.map(
          (tick, i) => html`
            <div class="tick">
              ${this.tickLabels
                ? html`
                    <div class="${this.tickLabelClassNames(i)}">
