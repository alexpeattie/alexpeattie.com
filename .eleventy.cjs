...
const { renumberFootnotes: remarkRenumberFootnotes } = require('@amanda-mitchell/remark-renumber-footnotes')
    ...
    plugins: [
      ...
      function directives() {
        ...
        function transform(tree) {
          visit(tree, { type: 'containerDirective', name: 'admonition' }, admonitionDirective)
          visit(tree, { type: 'leafDirective', name: 'figure' }, figureDirective)
        }
        ...
        function figureDirective(node, index, parent) {
          ...
          node.type = 'figure'
          data.hName = 'figure'
          node.children = [u('image', node.attributes), u('figcaption', { data: { hName: 'figcaption' } }, [caption])]
        }
      },
      ...
      function highlightCode() {
        ...
        function transformer(tree) {
          ...
          function visitor(node, i, parent) {
            ...
            if(node.frontmatter.filename) parent.children.splice(i, 0, {
              type: 'html',
      ...
      function inlineCodeFix() {
        ...
        function transformer(tree) {
          ...
          function visitor(node, _index, outerEl) {
            if(matches('a', outerEl)) outerEl.properties.className = [...(outerEl.properties.className || []), 'has-code']
          }
        }
      },
      function addFootnoteHeader() {
        ...
        function transformer(tree) {
          ...
          if(footnotesDiv) {
            const hrIndex = footnotesDiv.children.findIndex(node => matches('hr', node))
